...
func modelPriceNotConfiguredError(modelName string, userId int) error {
	isAdmin, err := model.IsAdmin(userId)
	if err != nil {
		// P4-2 修复：DB error 必须记录，不能静默当 false 处理（会误导用户提示信息）。
		common.SysError(fmt.Sprintf("IsAdmin query failed: user_id=%d, model=%s, error=%v", userId, modelName, err))
		return fmt.Errorf(
			"模型 %s 的价格未配置，且查询用户权限失败，请联系管理员；"+
				"Model %s price not configured and failed to query user permission. Please contact the site administrator.",
			modelName, modelName,
		)
	}
	if isAdmin {
