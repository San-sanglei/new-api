...
func ResponsesHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if info.RelayMode == relayconstant.RelayModeResponsesCompact {
		...
		if err := service.PostTextConsumeQuota(c, info, usageDto, nil); err != nil {
			common.SysError(fmt.Sprintf("ResponsesHandler: PostTextConsumeQuota failed (response already sent): user_id=%d, channel_id=%d, token_id=%d, err=%v",
	...
	if strings.HasPrefix(info.OriginModelName, "gpt-4o-audio") {
		...
		if err := service.PostAudioConsumeQuota(c, info, usageDto, ""); err != nil {
			common.SysError(fmt.Sprintf("ResponsesHandler: PostAudioConsumeQuota failed (response already sent): user_id=%d, channel_id=%d, token_id=%d, err=%v",
				info.UserId, info.ChannelId, info.TokenId, err))
		}
	} else {
		// P4-5 修复：PostTextConsumeQuota 现在返回 error。响应已返回给用户，
