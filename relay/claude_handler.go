...
func ClaudeHelper(c *gin.Context, info *relaycommon.RelayInfo) (newAPIError *types.NewAPIError) {
	...
	if !model_setting.GetGlobalSettings().PassThroughRequestEnabled &&
		...
		if err := service.PostTextConsumeQuota(c, info, usage, nil); err != nil {
			common.SysError(fmt.Sprintf("ClaudeViaResponses: PostTextConsumeQuota failed (response already sent): user_id=%d, channel_id=%d, token_id=%d, err=%v",
	...
	if err := service.PostTextConsumeQuota(c, info, usage.(*dto.Usage), nil); err != nil {
		common.SysError(fmt.Sprintf("ClaudeHandler: PostTextConsumeQuota failed (response already sent): user_id=%d, channel_id=%d, token_id=%d, err=%v",
			info.UserId, info.ChannelId, info.TokenId, err))
	}
	return nil
}
