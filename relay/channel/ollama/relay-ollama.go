...
func PullOllamaModel(baseURL, apiKey, modelName string) error {
	...
	// P1-2 修复：使用 NewRequestWithContext
	request, err := http.NewRequestWithContext(context.Background(), "POST", url, strings.NewReader(string(requestBody)))
	if err != nil {
...
func DeleteOllamaModel(baseURL, apiKey, modelName string) error {
	...
	// P1-2 修复：使用 NewRequestWithContext
	request, err := http.NewRequestWithContext(context.Background(), "DELETE", url, strings.NewReader(string(requestBody)))
	if err != nil {
		return fmt.Errorf("创建请求失败: %v", err)
	}
