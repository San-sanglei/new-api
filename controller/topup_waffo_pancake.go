...
func RequestWaffoPancakePay(c *gin.Context) {
	...
	if err != nil {
		...
		if updateErr := topUp.Update(); updateErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo Pancake 创建结账会话失败后更新订单状态为 Failed 失败 user_id=%d trade_no=%s error=%q", id, tradeNo, updateErr.Error()))
		}
...
func WaffoPancakeWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 收到请求 path=%q client_ip=%s body_len=%d has_signature=%t", c.Request.RequestURI, c.ClientIP(), len(bodyBytes), signature != ""))

	event, err := service.VerifyConfiguredWaffoPancakeWebhook(string(bodyBytes), signature)
	if err != nil {
		// P1-4 修复：禁止打印完整 webhook body 和 signature
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo Pancake webhook 验签失败 path=%q client_ip=%s body_len=%d error=%q", c.Request.RequestURI, c.ClientIP(), len(bodyBytes), err.Error()))
