...
func SubscriptionRequestWaffoPancakePay(c *gin.Context) {
	...
	if err != nil {
		...
		if updateErr := order.Update(); updateErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo Pancake 订阅结账会话创建失败后更新订单状态为 Failed 失败 user_id=%d plan_id=%d trade_no=%s error=%q", userId, plan.Id, tradeNo, updateErr.Error()))
		}
