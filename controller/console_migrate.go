...
func MigrateConsoleSetting(c *gin.Context) {
	...
	if v := valMap["ApiInfo"]; v != "" {
		...
		if err := json.Unmarshal([]byte(v), &arr); err == nil {
			...
			if err := model.UpdateOption("console_setting.api_info", string(bytes)); err != nil {
				common.SysError("failed to migrate ApiInfo: " + err.Error())
			}
		}
		if err := model.UpdateOption("ApiInfo", ""); err != nil {
			common.SysError("failed to clear ApiInfo: " + err.Error())
		}
	}
	// Announcements 直接搬
	if v := valMap["Announcements"]; v != "" {
		if err := model.UpdateOption("console_setting.announcements", v); err != nil {
			common.SysError("failed to migrate Announcements: " + err.Error())
		}
		if err := model.UpdateOption("Announcements", ""); err != nil {
			common.SysError("failed to clear Announcements: " + err.Error())
		}
	}
	// FAQ 转换
	if v := valMap["FAQ"]; v != "" {
		...
		if err := json.Unmarshal([]byte(v), &arr); err == nil {
			...
			if err := model.UpdateOption("console_setting.faq", string(bytes)); err != nil {
				common.SysError("failed to migrate FAQ: " + err.Error())
			}
		}
		if err := model.UpdateOption("FAQ", ""); err != nil {
			common.SysError("failed to clear FAQ: " + err.Error())
		}
	}
	...
	if url != "" && slug != "" {
		...
		if err := model.UpdateOption("console_setting.uptime_kuma_groups", string(bytes)); err != nil {
			common.SysError("failed to migrate UptimeKuma groups: " + err.Error())
		}
	}
	// 清空旧键内容
	if url != "" {
		if err := model.UpdateOption("UptimeKumaUrl", ""); err != nil {
			common.SysError("failed to clear UptimeKumaUrl: " + err.Error())
		}
	}
	if slug != "" {
		if err := model.UpdateOption("UptimeKumaSlug", ""); err != nil {
			common.SysError("failed to clear UptimeKumaSlug: " + err.Error())
		}
	}

	// 删除旧键记录
	oldKeys := []string{"ApiInfo", "Announcements", "FAQ", "UptimeKumaUrl", "UptimeKumaSlug"}
	if err := model.DB.Where("key IN ?", oldKeys).Delete(&model.Option{}).Error; err != nil {
		common.SysError("failed to delete old option keys: " + err.Error())
	}
