...
func Disable2FA(c *gin.Context) {
	...
	if err == nil {
		// 尝试验证TOTP
		// P4-5 修复：ValidateTOTPAndUpdateUsage 在 DB 失败时返回 error，必须 fail-closed。
		isValidTOTP, err = twoFA.ValidateTOTPAndUpdateUsage(cleanCode)
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
...
func Verify2FALogin(c *gin.Context) {
	...
	if err == nil {
		// 尝试验证TOTP
		// P4-5 修复：ValidateTOTPAndUpdateUsage 在 DB 失败时返回 error，
		// 必须失败关闭拒绝登录，避免暴力破解防护被绕过。
		isValidTOTP, err = twoFA.ValidateTOTPAndUpdateUsage(cleanCode)
		if err != nil {
			c.JSON(http.StatusOK, gin.H{
				"success": false,
