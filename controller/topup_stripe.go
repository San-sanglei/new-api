...
func (*StripeAdaptor) RequestPay(c *gin.Context, req *StripePayRequest) {
	...
	if err != nil {
		// P4-2 修复：禁止丢弃 DB error。GetUserById 失败时 user 为 nil，后续 *user 解引用会 panic，
		// 且 chargedMoney 会基于零值计算导致金额错误。
		common.SysError("Stripe 充值获取用户失败 user_id=" + strconv.Itoa(id) + " err=" + err.Error())
...
func StripeWebhook(c *gin.Context) {
	...
	// P1-4 修复：禁止打印完整 webhook body 和 signature，仅记录请求元数据
	logger.LogInfo(ctx, fmt.Sprintf("Stripe webhook 收到请求 path=%q client_ip=%s body_len=%d has_signature=%t", c.Request.RequestURI, c.ClientIP(), len(payload), signature != ""))
...
func sessionAsyncPaymentFailed(ctx context.Context, event stripe.Event, callerIp string) {
	...
	if err != nil {
		...
		// 注意：此函数为异步处理（无 gin.Context），无法返回 HTTP 响应，仅记录 SysError 并终止。
		common.SysError("Stripe 异步支付失败回调查询订单 DB 错误 trade_no=" + referenceId + " err=" + err.Error())
		return
	}
	if topUp == nil {
