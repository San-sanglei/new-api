...
func SyncUpstreamModels(c *gin.Context) {
	...
	go func() {
		...
		defer func() {
			if r := recover(); r != nil {
				fetchErr = fmt.Errorf("fetch vendors panic recovered: %v", r)
			}
		}()
		// vendor 失败不拦截
	...
	go func() {
		...
		defer func() {
			if r := recover(); r != nil {
				fetchErr = fmt.Errorf("fetch models panic recovered: %v", r)
			}
		}()
	...
	if len(req.Overwrite) > 0 {
		...
		for _, ow := range req.Overwrite {
			...
			if err := model.DB.Transaction(func(tx *gorm.DB) error {
				...
				common.SysError(fmt.Sprintf("sync upstream models: failed to update model %s: %s", ow.ModelName, err.Error()))
				c.JSON(http.StatusOK, gin.H{"success": false, "message": "更新模型失败: " + err.Error()})
				return
			}
		}
	}
...
func SyncUpstreamPreview(c *gin.Context) {
	...
	if len(upstreamNames) > 0 {
		// P3-5 修复：Find 错误必须记录，避免 DB 异常时 locals 为空导致预览状态全部显示"未配置"。
		if err := model.DB.Where("model_name IN ? AND sync_official <> 0", upstreamNames).Find(&locals).Error; err != nil {
			common.SysError(fmt.Sprintf("SyncUpstreamPreview: failed to load local models: %s", err.Error()))
		}
	}
	...
	if len(vendorIDs) > 0 {
		...
		// P3-5 修复：Find 错误必须记录，避免 DB 异常时供应商名称映射为空。
		if err := model.DB.Where("id IN ?", vendorIDs).Find(&dbVendors).Error; err != nil {
			common.SysError(fmt.Sprintf("SyncUpstreamPreview: failed to load vendors: %s", err.Error()))
		}
