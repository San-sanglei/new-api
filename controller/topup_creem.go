...
func (*CreemAdaptor) RequestPay(c *gin.Context, req *CreemPayRequest) {
	...
	if err != nil {
		// P4-2 修复：禁止丢弃 DB error。GetUserById 失败时 user 为 nil，后续 user.Id/Email/Username 解引用会 panic。
		common.SysError("Creem 充值获取用户失败 user_id=" + strconv.Itoa(id) + " err=" + err.Error())
...
func CreemWebhook(c *gin.Context) {
	...
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem webhook 收到请求 path=%q client_ip=%s body_len=%d has_signature=%t", c.Request.RequestURI, c.ClientIP(), len(bodyBytes), signature != ""))
	if signature == "" {
	...
	if !verifyCreemSignature(string(bodyBytes), signature, setting.CreemWebhookSecret) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Creem webhook 验签失败 path=%q client_ip=%s body_len=%d", c.Request.RequestURI, c.ClientIP(), len(bodyBytes)))
		c.AbortWithStatus(http.StatusUnauthorized)
		return
	}
	...
	if err := c.ShouldBindJSON(&webhookEvent); err != nil {
		// P1-4 修复：禁止打印完整 webhook body，仅记录错误和长度
		logger.LogError(c.Request.Context(), fmt.Sprintf("Creem webhook 解析失败 path=%q client_ip=%s body_len=%d error=%q", c.Request.RequestURI, c.ClientIP(), len(bodyBytes), err.Error()))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
...
func handleCheckoutCompleted(c *gin.Context, event *CreemWebhookEvent) {
	...
	// P1-4 修复：客户邮箱/姓名脱敏，禁止明文输出 PII
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Creem 支付完成回调 trade_no=%s creem_order_id=%s amount_paid=%d currency=%s product_name=%q customer_email=%s", referenceId, event.Object.Order.Id, event.Object.Order.AmountPaid, event.Object.Order.Currency, event.Object.Product.Name, common.MaskEmail(event.Object.Customer.Email)))

	// 查询本地订单确认存在
	topUp, err := model.GetTopUpByTradeNo(referenceId)
	if err != nil {
		// P4-2：DB 查询错误必须终止回调流程，避免在 DB 异常时误判订单不存在或使用零值继续处理。
		common.SysError("Creem 充值回调查询订单 DB 错误 trade_no=" + referenceId + " creem_order_id=" + event.Object.Order.Id + " err=" + err.Error())
		c.AbortWithStatus(http.StatusInternalServerError)
		return
	}
	if topUp == nil {
...
func genCreemLink(ctx context.Context, referenceId string, product *CreemProduct, email string, username string) (string, error) {
	...
	// P1-4 修复：邮箱脱敏
	logger.LogInfo(ctx, fmt.Sprintf("Creem 支付请求已发送 api_url=%s product_id=%s email=%s trade_no=%s", apiUrl, product.ProductId, common.MaskEmail(email), referenceId))
	...
	// P1-4 修复：禁止打印完整 API 响应体（可能含 checkout URL、客户信息），仅记录状态码和长度
