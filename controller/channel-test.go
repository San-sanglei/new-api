...
func testChannel(channel *model.Channel, testUserID int, testModel string, endpointType string, isStream bool) testResult {
	...
	if err := model.RecordConsumeLog(c, testUserID, model.RecordConsumeLogParams{
		...
		// P4-5: 测试消费日志写入失败不阻塞测试流程，但必须 SysError 记录上下文供对账。
		// 测试 quota 由 testAllChannels 流程结算，若日志未落库，运维需通过本 SysError 核对测试扣费。
		common.SysError(fmt.Sprintf(
			"testChannel: consume log persist failed: log_persist_required=true, test_user_id=%d, channel_id=%d, model_name=%s, quota=%d, log_type=%d, err=%v",
			testUserID, channel.Id, info.OriginModelName, quota, model.LogTypeConsume, err,
		))
	}
	// P1-4 修复：禁止打印完整上游响应体（可能回显 API Key），仅记录元数据
	common.SysLog(fmt.Sprintf("testing channel #%d completed, response_len=%d", channel.Id, len(respBody)))
	return testResult{
		context:     c,
		localErr:    nil,
		newAPIError: nil,
...
// AutomaticallyTestChannels 定时自动测试所有渠道。
	...
	autoTestChannelsOnce.Do(func() {
		for {
			if !operation_setting.GetMonitorSetting().AutoTestChannelEnabled {
				if !common.SleepWithContext(ctx, 1*time.Minute) {
					common.SysLog("AutomaticallyTestChannels stopping: context cancelled")
					return
				}
				continue
			}
			for {
				...
				if !common.SleepWithContext(ctx, time.Duration(int(math.Round(frequency)))*time.Minute) {
					common.SysLog("AutomaticallyTestChannels stopping: context cancelled")
					return
				}
				common.SysLog(fmt.Sprintf("automatically test channels with interval %f minutes", frequency))
				common.SysLog("automatically testing all channels")
				// P4-5：原 `_ = testAllChannels(false)` 吞掉 error，自动测试周期失败时调度器无感知。
				//   testAllChannels 内部已捕获 GetChannels / resolveChannelTestUserID / 单渠道测试错误并返回聚合 err，
				//   此处仅 SysError 暴露状态，不阻断调度器循环，下一周期仍会按 frequency 重试。
				if err := testAllChannels(false); err != nil {
					common.SysError(fmt.Sprintf(
						"testAllChannels failed: automated_test_required=true err=%v",
						err,
					))
				}
