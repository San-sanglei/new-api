...
// StartChannelUpstreamModelUpdateTask 启动渠道上游模型更新任务。
// P1 修复：接受 ctx，服务退出时可通过 ctx.Done() 停止循环。
func StartChannelUpstreamModelUpdateTask(ctx context.Context) {
	channelUpstreamModelUpdateTaskOnce.Do(func() {
		...
		common.GoSafeWithContext(func(ctx context.Context) {
			...
			for {
				select {
				case <-ctx.Done():
					common.SysLog("upstream model update task stopping: context cancelled")
					return
				case <-ticker.C:
					runChannelUpstreamModelUpdateTaskOnce()
				}
