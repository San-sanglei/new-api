...
func UniversalVerify(c *gin.Context) {
	...
	case "2fa":
		...
		// P4-5 修复：validateTwoFactorAuth 返回 (bool, error)，DB 失败时 fail-closed。
		verified, err = validateTwoFactorAuth(twoFA, req.Code)
		if err != nil {
			common.ApiError(c, fmt.Errorf("验证服务暂时不可用，请稍后重试"))
			return
		}
		verifyMethod = "2FA"
