...
func AdminResetPasskey(c *gin.Context) {
	...
	// 使用 GetUserById 而非 FillUserById：FillUserById 内部忽略 First() 错误，
	// 对不存在的用户 id 会返回 nil 错误且不填充用户数据（Role 保持零值），
	// 导致 canManageTargetRole 检查被绕过并返回误导性的 "该用户尚未绑定 Passkey" 消息。
	// GetUserById 会正确返回 gorm.ErrRecordNotFound，与 AdminDisable2FA 行为一致。
