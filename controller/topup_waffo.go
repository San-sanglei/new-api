...
func RequestWaffoPay(c *gin.Context) {
	...
	group, err := model.GetUserGroup(id, true)
	if err != nil {
		// P4-2 修复：禁止丢弃 DB error。GetUserGroup 失败时 group=""，
		// 会导致 getWaffoPayMoney 基于空分组计算支付金额，定价错误。
		common.SysError("Waffo 充值获取用户分组失败 user_id=" + strconv.Itoa(id) + " err=" + err.Error())
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "获取用户分组失败"})
		return
	}
	payMoney := getWaffoPayMoney(float64(req.Amount), group)
	...
	if err != nil {
		...
		// P4-2 修复：状态更新失败必须记录，避免订单滞留 Pending 且无日志可循。
		if updateErr := topUp.Update(); updateErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo SDK 初始化失败后更新订单状态为 Failed 失败 user_id=%d trade_no=%s error=%q", id, merchantOrderId, updateErr.Error()))
		}
	...
	if err != nil {
		...
		// P4-2 修复：状态更新失败必须记录，避免订单滞留 Pending 且无日志可循。
		if updateErr := topUp.Update(); updateErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo 创建订单失败后更新订单状态为 Failed 失败 user_id=%d trade_no=%s error=%q", id, merchantOrderId, updateErr.Error()))
		}
	...
	if !resp.IsSuccess() {
		...
		// P4-2 修复：状态更新失败必须记录，避免订单滞留 Pending 且无日志可循。
		if updateErr := topUp.Update(); updateErr != nil {
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo 业务失败后更新订单状态为 Failed 失败 user_id=%d trade_no=%s error=%q", id, merchantOrderId, updateErr.Error()))
		}
		c.JSON(http.StatusOK, gin.H{"message": "error", "data": "拉起支付失败"})
		return
	}
...
func WaffoWebhook(c *gin.Context) {
	...
	// P1-4 修复：禁止打印完整 webhook body 和 signature，仅记录元数据
	logger.LogInfo(c.Request.Context(), fmt.Sprintf("Waffo webhook 收到请求 path=%q client_ip=%s body_len=%d has_signature=%t", c.Request.RequestURI, c.ClientIP(), len(bodyBytes), signature != ""))

	// 验证请求签名
	if !wh.VerifySignature(bodyStr, signature) {
		logger.LogWarn(c.Request.Context(), fmt.Sprintf("Waffo webhook 验签失败 path=%q client_ip=%s body_len=%d", c.Request.RequestURI, c.ClientIP(), len(bodyBytes)))
		c.AbortWithStatus(http.StatusBadRequest)
		return
	}
	...
	if err := common.Unmarshal(bodyBytes, &event); err != nil {
		// P1-4 修复：禁止打印完整 webhook body
		logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo webhook 解析失败 path=%q client_ip=%s body_len=%d error=%q", c.Request.RequestURI, c.ClientIP(), len(bodyBytes), err.Error()))
	...
	case core.EventPayment:
		...
		if err := common.Unmarshal(bodyBytes, &payload); err != nil {
			// P1-4 修复：禁止打印完整支付回调载荷
			logger.LogError(c.Request.Context(), fmt.Sprintf("Waffo 支付回调载荷解析失败 event_type=%s client_ip=%s body_len=%d error=%q", event.EventType, c.ClientIP(), len(bodyBytes), err.Error()))
