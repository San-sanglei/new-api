...
interface AnnouncementItem {
  ...
  /** Optional category for richer icon rendering (mock notifications) */
  category?: 'balance' | 'channel' | 'system' | 'feature'
  /** Optional title shown above the content (mock notifications) */
  title?: string
}
...
/**
 * Icon + tint for a notification category. Falls back to a colored dot when no
 * category is present (real announcements from the API).
 */
const CATEGORY_ICON: Record<
  NonNullable<AnnouncementItem['category']>,
  { icon: React.ComponentType<{ className?: string }> }
> = {
  balance: { icon: Wallet },
  channel: { icon: AlertTriangle },
...
/**
 * Announcement status indicator — renders a category icon when available,
 * otherwise falls back to a colored dot.
 */
function AnnouncementIndicator({ item }: { item: AnnouncementItem }) {
...
function NoticeContent({
  ...
  // Show content as soon as any is available. When the backend is down, the
  // notice string is empty — show a graceful empty state instead of an
...
function AnnouncementsContent({
  ...
  // Show content as soon as any is available — mock/demo notifications are
  // synchronous, so the Timeline tab must render them immediately even while
  // the backend is still loading (or down). The loading state only matters
  // when we have nothing to show at all.
            ...
            <div key={idx}>
              <div className='py-3'>
                <div className='flex items-start gap-3'>
                  <AnnouncementIndicator item={item} />
