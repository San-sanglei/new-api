...
import { DEFAULT_PRESET_MULTIPLIERS } from '../constants'
import type {
  TopupInfo,
  PresetAmount,
  CreemProduct,
...
export function useTopupInfo() {
  ...
  const fetchTopupInfo = async () => {
    try {
      ...
      // Always use fixed preset amounts: 10, 50, 100, 500
      const fixedPresets = DEFAULT_PRESET_MULTIPLIERS.map((value) => ({
        value,
      }))
      setPresetAmounts(fixedPresets)
    } catch (err) {
      // eslint-disable-next-line no-console
