...
export function useNotifications() {
  ...
  // Default to the announcements tab so mock/demo notifications display
  // immediately on open — the notice tab depends on the backend and would
  // otherwise show a loading state when the API is unavailable.
  ...
  const liveAnnouncements: Record<string, unknown>[] = announcementsEnabled
    ? ((status?.announcements || []) as Record<string, unknown>[]).slice(0, 20)
    : []

  // Merge demo/mock notifications into the feed so the notification center
  // always has content to display. Mock items carry stable numeric ids so the
  // read-state store tracks them like real announcements.
  const announcements: Record<string, unknown>[] = useMemo(
    () => [...mockNotifications, ...liveAnnouncements],
    [liveAnnouncements]
  )
  ...
  // Explicitly mark every notification (notice + all announcements) as read.
  // Wired to the "Mark all read" button in the popover footer.
  const markAllRead = () => {
    if (noticeContent) markNoticeRead(noticeContent)
