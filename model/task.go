...
// TaskGetAllUserTask 返回指定用户的任务列表。
// P4-5 修复：原签名 []*Task 在 DB 异常时返回 nil，调用方无法区分"无任务"与"DB 故障"。
...
// GetTimedOutUnfinishedTasks 返回超时未完成的任务列表。
// P4-2 修复：返回 ([]*Task, error) 以区分"无任务"与"DB 查询错误"。
// - 查询成功（含空结果）：返回 (tasks, nil)
...
// GetAllUnFinishSyncTasks 返回所有未完成的同步任务。
// P4-2 修复：返回 ([]*Task, error) 以区分"无任务"与"DB 查询错误"。
// - 查询成功（含空结果）：返回 (tasks, nil)
// - DB error：返回 (nil, error)，调用方必须处理
...
func TaskCountAllTasks(queryParams SyncTaskQueryParams) int64 {
	...
	// P3-5 修复：Count 错误必须记录，避免 DB 异常时静默返回 0。
	if err := query.Count(&total).Error; err != nil {
		common.SysError(fmt.Sprintf("TaskCountAllTasks query failed: error=%s", err.Error()))
	}
	return total
}
...
func TaskCountAllUserTask(userId int, queryParams SyncTaskQueryParams) int64 {
	...
	// P3-5 修复：Count 错误必须记录，避免 DB 异常时静默返回 0。
	if err := query.Count(&total).Error; err != nil {
		common.SysError(fmt.Sprintf("TaskCountAllUserTask query failed: user_id=%d, error=%s", userId, err.Error()))
	}
	return total
}
