...
// MaxPrefillGroupResults 限制预填组列表查询的最大返回记录数，
// 防止数据增长后全量加载。
const MaxPrefillGroupResults = 500
...
func GetAllPrefillGroups(groupType string) ([]*PrefillGroup, error) {
	...
	// 强制加上限，防止数据增长后全量加载导致慢查询
	if err := query.Order("updated_time DESC").Limit(MaxPrefillGroupResults).Find(&groups).Error; err != nil {
		return nil, err
	}
	return groups, nil
}
