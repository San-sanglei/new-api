...
// UpdateQuotaData 周期性将缓存中的看板数据写入数据库。
...
func UpdateQuotaData(ctx context.Context) {
	for {
		...
		if !common.SleepWithContext(ctx, time.Duration(common.DataExportInterval)*time.Minute) {
			common.SysLog("UpdateQuotaData stopping: context cancelled")
			return
		}
	}
}
...
func SaveQuotaDataCache() {
	...
	for _, quotaData := range CacheQuotaData {
		...
		if err != nil && !errors.Is(err, gorm.ErrRecordNotFound) {
			// First 查询失败（非"未找到"），记录错误并跳过本条，避免基于零值 Id 误判为"不存在"导致重复插入。
			common.SysError(fmt.Sprintf("SaveQuotaDataCache: failed to query quota_data (user_id=%d, model_name=%s, created_at=%d): %s",
				quotaData.UserID, quotaData.ModelName, quotaData.CreatedAt, err.Error()))
			failedCount++
			continue
		}
		if quotaDataDB.Id > 0 {
			...
			if err := DB.Table("quota_data").Create(quotaData).Error; err != nil {
				common.SysError(fmt.Sprintf("SaveQuotaDataCache: failed to create quota_data (user_id=%d, model_name=%s, created_at=%d): %s",
					quotaData.UserID, quotaData.ModelName, quotaData.CreatedAt, err.Error()))
				failedCount++
			}
		}
	...
	if failedCount > 0 {
		common.SysError(fmt.Sprintf("保存数据看板数据完成，共%d条，失败%d条", size, failedCount))
	} else {
