...
func updatePricing() {
	...
	var allMeta []Model
	// P3-5 修复：Find 错误必须记录，避免 DB 异常时 metaMap 为空影响定价展示。
	if err := DB.Find(&allMeta).Error; err != nil {
		common.SysError(fmt.Sprintf("updatePricing: failed to load model meta: %s", err.Error()))
	}
	...
	// 预加载供应商
	var vendors []Vendor
	// P3-5 修复：Find 错误必须记录，避免 DB 异常时 vendorMap 为空影响供应商展示。
	if err := DB.Find(&vendors).Error; err != nil {
		common.SysError(fmt.Sprintf("updatePricing: failed to load vendors: %s", err.Error()))
	}
