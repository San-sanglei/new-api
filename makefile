...
.PHONY: all build-frontend build-frontend-classic build-all-frontends start-backend dev dev-api dev-api-rebuild dev-web dev-web-classic reset-setup test test-backend test-race vet lint-frontend build-frontend-check
...
test: test-backend lint-frontend build-frontend-check
...
lint-frontend:
	@echo "Linting default frontend..."
	@cd ./web && bun install --frozen-lockfile
	@cd $(FRONTEND_DIR) && bun run lint

build-frontend-check:
	@echo "Typecheck + build default frontend..."
	@cd ./web && bun install --frozen-lockfile
	@cd $(FRONTEND_DIR) && DISABLE_ESLINT_PLUGIN='true' bun run build
