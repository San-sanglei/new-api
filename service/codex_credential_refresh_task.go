...
// StartCodexCredentialAutoRefreshTask 启动 Codex 凭证自动刷新任务。
// P1 修复：接受 ctx，服务退出时可通过 ctx.Done() 停止循环。
func StartCodexCredentialAutoRefreshTask(ctx context.Context) {
	codexCredentialRefreshOnce.Do(func() {
		...
		common.GoSafeWithContext(func(ctx context.Context) {
			...
			for {
				select {
				case <-ctx.Done():
					logger.LogInfo(ctx, "codex credential auto-refresh task stopping: context cancelled")
					return
				case <-ticker.C:
					runCodexCredentialAutoRefreshOnce()
				}
	...
	if refreshed > 0 {
		func() {
			...
			if err := model.InitChannelCache(); err != nil {
				logger.LogError(ctx, fmt.Sprintf("codex credential auto-refresh: InitChannelCache failed: %v", err))
			}
		}()
