...
func ResolveWaffoPancakeTradeNo(event *WaffoPancakeWebhookEvent) (string, error) {
	...
	if err != nil {
		// P4-2：DB 查询错误必须终止，避免在 DB 异常时误判订单不存在或使用零值继续处理。
		return "", fmt.Errorf("waffo pancake query topup db error for tradeNo=%s: %w", tradeNo, err)
	}
...
func ResolveWaffoPancakeSubscriptionTradeNo(event *WaffoPancakeWebhookEvent) (string, error) {
	...
	if err != nil {
		// P4-2：DB 查询错误必须终止，避免在 DB 异常时误判订单不存在或使用零值继续处理。
		return "", fmt.Errorf("waffo pancake query subscription order db error for tradeNo=%s: %w", tradeNo, err)
	}
