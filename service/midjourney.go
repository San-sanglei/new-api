...
func DoMidjourneyHttpRequest(c *gin.Context, timeout time.Duration, fullRequestURL string) (*dto.MidjourneyResponseWithStatusCode, []byte, error) {
	...
	// P1-2 修复：使用 NewRequestWithContext 传递 ctx，用户取消请求时上游请求同步取消
	req, err := http.NewRequestWithContext(c.Request.Context(), c.Request.Method, fullRequestURL, strings.NewReader(string(reqBody)))
	if err != nil {
