...
func NotifyRootUser(t string, subject string, content string) {
	...
	if err != nil {
		// P4-2 修复：DB error 必须记录，避免静默跳过 root 通知（掩盖系统故障）。
		common.SysError(fmt.Sprintf("failed to notify root user: get root user db error: %s", err.Error()))
		return
	}
	if user == nil {
	...
	if err != nil {
		// P4-5：root 通知发送失败仅 SysLog 无结构化字段，运维无法在告警系统中关联具体失败事件。
		//   通知失败本身不阻断业务（DB 状态已落库），升级为 SysError 触发告警便于人工补发。
		common.SysError(fmt.Sprintf(
			"user notification failed: user_id=%d notification_required=true err=%v",
			baseUser.Id, err,
		))
	}
}
...
func NotifyUpstreamModelUpdateWatchers(subject string, content string) {
	...
	if err := model.DB.
		...
		// P4-5：DB 查询失败导致整个批量通知无法发出，升级为 SysError 触发告警。
		common.SysError(fmt.Sprintf(
			"user notification failed: notification_required=true err=%v",
			err,
		))
		return
	}
	...
	for _, user := range users {
		...
		if err := NotifyUser(user.Id, user.Email, userSetting, notification); err != nil {
			// P4-5：批量通知中单个用户发送失败仅 SysLog，无 users_count 上下文，
			//   升级为 SysError 暴露 user_id + users_count + err，便于运维核对哪些管理员漏收。
			common.SysError(fmt.Sprintf(
				"user notification failed: user_id=%d users_count=%d notification_required=true err=%v",
				user.Id, usersCount, err,
			))
			continue
		}
		sentCount++
...
func NotifyUser(userId int, userEmail string, userSetting dto.UserSetting, data dto.Notify) error {
	...
	if err != nil {
		// P4-5：限流检查失败导致通知无法发出，升级为 SysError 暴露 user_id + err 供告警。
		common.SysError(fmt.Sprintf(
			"user notification failed: user_id=%d notification_required=true err=%v",
			userId, err,
		))
...
func sendGotifyNotify(gotifyUrl string, gotifyToken string, priority int, data dto.Notify) error {
	...
	if system_setting.EnableWorker() {
		...
		// 直接发送请求
		// P1-2 修复：使用 NewRequestWithContext
		req, err = http.NewRequestWithContext(context.Background(), http.MethodPost, finalURL, bytes.NewBuffer(payloadBytes))
		if err != nil {
