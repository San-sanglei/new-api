...
func ChargeViolationFeeIfNeeded(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, apiErr *types.NewAPIError) bool {
	...
	if err := model.RecordConsumeLog(ctx, relayInfo.UserId, model.RecordConsumeLogParams{
		...
		// P4-5: 违规扣费审计日志写入失败不阻塞扣费流程，但必须 SysError 记录完整上下文供对账。
		// Other 字段含 CSAMViolationMarker / violation_fee_code 等合规审计关键信息，若日志未落库，
		//   合规审计追溯将断裂，运维需通过本 SysError 立即人工对账。
		common.SysError(fmt.Sprintf(
			"violation_fee: consume log persist failed: log_persist_required=true, user_id=%d, token_id=%d, channel_id=%d, model_name=%s, quota=%d, log_type=%d, err=%v",
			relayInfo.UserId, relayInfo.TokenId, relayInfo.ChannelId, relayInfo.OriginModelName, feeQuota, model.LogTypeConsume, err,
		))
	}
