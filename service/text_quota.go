...
func PostTextConsumeQuota(ctx *gin.Context, relayInfo *relaycommon.RelayInfo, usage *dto.Usage, extraContent []string) error {
	...
	// P4-5 修复：SettleBilling 失败时禁止 LogError 后继续。
	// 原逻辑仅 logger.LogError 后继续 RecordConsumeLog 并正常返回，导致：
	...
	if err := SettleBilling(ctx, relayInfo, summary.Quota); err != nil {
		common.SysError(fmt.Sprintf("PostTextConsumeQuota: SettleBilling failed: user_id=%d, channel_id=%d, token_id=%d, model=%s, quota=%d, err=%v",
			relayInfo.UserId, relayInfo.ChannelId, relayInfo.TokenId, summary.ModelName, summary.Quota, err))
		settleErr = err
	}
	...
	if err := model.RecordConsumeLog(ctx, relayInfo.UserId, model.RecordConsumeLogParams{
		...
		// Other 字段含 input_tokens_total / tiered billing 等关键计费信息，若日志未落库，
		...
		common.SysError(fmt.Sprintf(
			...
			relayInfo.UserId, relayInfo.TokenId, relayInfo.ChannelId, logModel, summary.Quota, model.LogTypeConsume, err,
		))
	}
	gopool.Go(func() {
