...
func sweepTimedOutTasks(ctx context.Context) {
	...
	if err != nil {
		// P4-2 修复：DB 查询错误必须记录，避免静默跳过超时任务处理（影响退款）。
		logger.LogError(ctx, fmt.Sprintf("sweepTimedOutTasks query timed out tasks db error: %v", err))
		return
	}
	if len(tasks) == 0 {
		return
...
func TaskPollingLoop(ctx context.Context) {
	for {
		if !common.SleepWithContext(ctx, time.Duration(15)*time.Second) {
			common.SysLog("TaskPollingLoop stopping: context cancelled")
			return
		}
		...
		if err != nil {
			// P4-2 修复：DB 查询错误必须记录，避免静默跳过任务同步轮询。
			logger.LogError(ctx, fmt.Sprintf("TaskPollingLoop query unfinished sync tasks db error: %v", err))
			continue
		}
