...
func SetupContextForToken(c *gin.Context, token *model.Token, parts ...string) error {
	...
	if len(parts) > 1 {
		isAdmin, err := model.IsAdmin(token.UserId)
		if err != nil {
			// P4-2 修复：DB error 必须记录，不能静默当 false 处理（会误剥夺管理员权限）。
			logger.LogError(c.Request.Context(), fmt.Sprintf("IsAdmin query failed: user_id=%d, error=%v", token.UserId, err))
			abortWithOpenAiMessage(c, http.StatusInternalServerError, "服务器内部错误")
			return fmt.Errorf("IsAdmin query failed: %w", err)
		}
		if isAdmin {
