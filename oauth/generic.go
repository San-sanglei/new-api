...
func (p *GenericOAuthProvider) ExchangeToken(ctx context.Context, code string, c *gin.Context) (*OAuthToken, error) {
	...
	bodyStr := string(body)
	// 不记录原始响应体，避免 access_token / refresh_token 泄露到日志
	logger.LogDebug(ctx, "[OAuth-Generic-%s] ExchangeToken response length: %d", p.config.Slug, len(bodyStr))
...
func (p *GenericOAuthProvider) GetUserInfo(ctx context.Context, token *OAuthToken) (*OAuthUser, error) {
	...
	bodyStr := string(body)
	// 不记录原始响应体，避免用户敏感信息泄露到日志
	logger.LogDebug(ctx, "[OAuth-Generic-%s] GetUserInfo response length: %d", p.config.Slug, len(bodyStr))
