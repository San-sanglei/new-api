...
func StartSystemMonitor(ctx context.Context) {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				SysError(fmt.Sprintf("StartSystemMonitor panic recovered: %v", r))
			}
		}()
		for {
			...
			if !config.Enabled {
				if !SleepWithContext(ctx, 30*time.Second) {
					SysLog("StartSystemMonitor stopping: context cancelled")
					return
				}
				continue
			}
