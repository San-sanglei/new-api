...
func InitRedisClient() (err error) {
	...
	// P1-2 修复：补充连接超时、读写超时、连接池配置，避免无限等待导致服务不可用
	opt.PoolSize = GetEnvOrDefault("REDIS_POOL_SIZE", 20)
	...
	opt.PoolTimeout = time.Duration(GetEnvOrDefault("REDIS_POOL_TIMEOUT", 4)) * time.Second
	opt.IdleTimeout = time.Duration(GetEnvOrDefault("REDIS_IDLE_TIMEOUT", 300)) * time.Second
	opt.MaxConnAge = time.Duration(GetEnvOrDefault("REDIS_MAX_CONN_AGE", 0)) * time.Second
	...
	if DebugEnabled {
		...
		SysLog(fmt.Sprintf("Redis pool size: %d, min idle conns: %d", opt.PoolSize, opt.MinIdleConns))
		SysLog(fmt.Sprintf("Redis dial timeout: %v, read timeout: %v, write timeout: %v", opt.DialTimeout, opt.ReadTimeout, opt.WriteTimeout))
	}
	return err
}
...
func RedisHIncrBy(key, field string, delta int64) error {
	...
	if err != nil && !errors.Is(err, redis.Nil) {
		// P3 Redis/DB 双写一致性修复：
		//   TTL 读取失败（非 redis.Nil）说明 Redis 异常，返回 error 让调用方感知缓存更新未生效，
		//   避免调用方误以为缓存已同步而 DB 失败时造成缓存与 DB 不一致。
		return fmt.Errorf("failed to get TTL: %w", err)
	}
	...
	// P3 修复：TTL <= 0（key 不存在或已过期 / 无 TTL）时设置默认 TTL，
	//   避免 key 永久残留或被静默跳过导致缓存与 DB 不一致。
	var expireTTL time.Duration
	if ttl > 0 {
		expireTTL = ttl
	} else {
		expireTTL = time.Duration(RedisKeyCacheSeconds()) * time.Second
	}
	txn.Expire(ctx, key, expireTTL)
